...
```c
// drivers/drv_adc_reader.h — 基础读取接口，大多数模块只需要这个
#pragma once
...
```c
// drivers/drv_adc_voltage_reader.h — 电压读取接口，安全监控/诊断需要
#pragma once
#include "drv_common.h"
...
```c
// drivers/drv_adc_continuous.h — DMA 连续采集接口，高速采集模块需要
#pragma once
#include "drv_common.h"
...
```c
// drivers/drv_common.h — 驱动层公共类型定义
#pragma once
#include <stdint.h>
#include "error.h"
...
void platform_init(void) {
    ...
    platform_register_can_port(&can_port_stm32g4);
...
typedef struct {
    ...
    // FW_OK    = 成功拿到一致性快照，seq 返回当前序号
    // FW_BUSY  = 当前没有新样本（调用者沿用上次快照或等待通知）
    // FW_ERROR = 底层采集异常
...
static const protocol_bundle_t js300_protocol = {
    ...
    .canopen = {
        .can     = NULL,     // 运行时由 platform_get_can_port() 填充
        .node_id = 0x10,
...
const can_port_t*      app_get_can_port(void)       { return platform_get_can_port(); }  // v5
...
static void main_task_entry(void *param) {
    ...
    for (;;) {
        ...
        if (device_state_get_current() == STATE_OPERATIONAL) {
            ...
            // FW_BUSY: 沿用上次快照继续处理（同步后端不会出现此情况）
...
│
├── common/                     # 全局公共定义（所有层共用）
│   └── error.h                 # fw_status_t 通用状态码
│
├── drivers/                    # 硬件抽象接口（只有 .h，ISP 拆分后）
│   ├── drv_common.h            # 驱动公共类型（adc_channel_id_t, gpio_pin_id_t）
